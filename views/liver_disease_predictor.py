...
def display_prediction(pred, prob):
    ...
    df_prob.columns = ['No Liver Disease', 'Liver Disease']
    if pred[0] == 0:
        prob = "{:.3f}".format(prob[0][0])
        st.success(f"Liver Disease: NO")
    else:
        prob = "{:.3f}".format(prob[0][0])
        st.error(f"Liver Disease: YES")

    st.subheader('Prediction Probability')
    st.dataframe(df_prob,
                 column_config={
                     'No Liver Disease': st.column_config.ProgressColumn(
                         'No Liver Disease',
                         format='%.2f',
                         width='medium',
                         min_value=0,
                     ...
                     'Liver Disease': st.column_config.ProgressColumn(
                         'Liver Disease',
                         format='%.2f',
                         width='medium',
                         min_value=0,
...
model, scalar, df_performance_metric, cm = train_model(model_name, 'Liver', 'binary')
